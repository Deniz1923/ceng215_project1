...
# This captures the non-uniform characteristic with flat and sloped
regions.
...
# dx2/dt = dvC/dt = iL / C
#
# IMPORTANT: When iL tries to go negative, the diode blocks (open
circuit)
# We must enforce iL >= 0 to model the diode's unidirectional
conduction.
...
ax1.axhline(y=Vi, color='r', linestyle='--', linewidth=1, label=f'Vi =
{Vi} V')
...
ax2.axhline(y=Vi, color='r', linestyle='--', linewidth=1, label=f'Vi =
{Vi} V')
